"""Block 5 final step: combine XBRL + prose facts into one Fact Store.

Reads:
  - data/fact_store/dev_xbrl_facts.jsonl   (Block 4 output)
  - data/fact_store/dev_prose_facts.jsonl  (Block 5 extraction output)
...
Pipeline:
  ...
  2. Rebuild the ChromaDB ``facts`` collection at data/fact_store/chromadb
     (clean rebuild — drops the XBRL-only collection from Block 4).
...
XBRL_JSONL = Path("data/fact_store/dev_xbrl_facts.jsonl")
PROSE_JSONL = Path("data/fact_store/dev_prose_facts.jsonl")
BM25_PATH = Path("data/fact_store/bm25.pkl")
LOG_PATH = Path("data/logs/dev_combined_fact_build.json")
...
def main() -> None:
    if not XBRL_JSONL.exists():
        raise SystemExit(f"Missing {XBRL_JSONL}. Run Block 4 first.")
